...
private:
  ...
  bool setHeader();
  void setContentType();
  bool fileStatRead(struct stat &, const Request &req,
                    const Config_Server &serv);
  bool generateDirectoryIndex(const char *dir_path, const Request &req);
  const char *matchRouteToRoot(const Request &req,
                               const std::vector<Config_Route> &routes);
};

// filepath == is path to the script
// eum e_scig is which interpreter should run it
//  for michal <your class> popen(const char * _file_path,cont char *
//  e_cgi_extension);
